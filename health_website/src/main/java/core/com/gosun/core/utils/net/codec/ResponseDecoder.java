...
public class ResponseDecoder extends CumulativeProtocolDecoder {
	...
	protected boolean doDecode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {
		...
		if(!this.isValidHead(in,position)) return false;
		
		contentLength = this.getContentLength(in,position);
	 ...
	 */
	private boolean isValidHead(IoBuffer in, int lastPosition){
		try {
		  ...
		  if(StringUtils.isBlank(inStr)||inStr.indexOf("GSSmp")<0){
				...
				if((headArray[0]).equalsIgnoreCase(Constants.gsCommonReponseHead))
				{
					...
					in.position(lastPosition+61+contentTypeLength);
				...
				if(!matched){
					if((headArray[0]).equalsIgnoreCase(Constants.gsCommonRequestHead))
					{
						...
						in.position(lastPosition+60+contentTypeLength);
	...
	private int getContentLength(IoBuffer in, int lastPosition){
		int length = 0;
		
		int position = in.position();
		int headBeforePackageSize = this.isRequestMessage ? (60+contentTypeLength) : (61+contentTypeLength);
		
		int contentPositon = position-4-headBeforePackageSize-lastPosition;//减去固定头部和尾部的\r\n
