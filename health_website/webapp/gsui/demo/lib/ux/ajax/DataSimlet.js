 ...
 * group summary).
 */
Gsui.define('Gsui.ux.ajax.DataSimlet', function () {
    ...
    return {
        ...
        getData: function (ctx) {
            ...
            ctx.filterSpec = params.filter && Gsui.decode(params.filter);
            ctx.groupSpec = params.group && Gsui.decode(params.group);

            fields = params.sort;
            if (params.dir) {
                fields = [{ direction: params.dir, property: fields }];
            } else {
                fields = Gsui.decode(params.sort);
            }

            if (ctx.filterSpec) {
                var filters = new Gsui.util.FilterCollection();
                filters.add(this.processFilters(ctx.filterSpec));
                data = Gsui.Array.filter(data, filters.getFilterFn());
            }
            ...
            data = Gsui.isArray(data) ? data.slice(0) : data;
            if (sortFn) {
                Gsui.Array.sort(data, sortFn);
            }
        ...
        processFilters: Gsui.identityFn,
