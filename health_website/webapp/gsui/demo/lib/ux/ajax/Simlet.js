 ...
 * to provide a response given a {@link Gsui.ux.ajax.SimXhr} instance.
 */
Gsui.define('Gsui.ux.ajax.Simlet', function () {
    var urlRegex = /([^?#]*)(#.*)?$/,
    ...
    return {
        ...
        doGet: function (ctx) {
            ...
            Gsui.Array.forEach(me.responseProps, function (prop) {
                if (prop in me) {
                    ret[prop] = me[prop];
                }
            });
         ...
         *
         * @param {Gsui.ux.ajax.SimXhr} xhr The simulated XMLHttpRequest instance.
         * @returns {Object} The response properties to add to the XMLHttpRequest.
         */
        exec: function (xhr) {
            var me = this,
                ret = {},
                method = 'do' + Gsui.String.capitalize(xhr.method.toLowerCase()), // doGet
                fn = me[method];
        ...
        redirect: function (method, url, params) {
            ...
            if (params) {
                url = Gsui.urlAppend(url, Gsui.Object.toQueryString(params));
            }
