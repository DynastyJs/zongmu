/**
 * @class Gsui.sparkline.Base
 *
 * The base class for ExtJS SparkLines. SparkLines are small, inline graphs used to visually
 * display small amounts of data. For large datasets, use the {@link Gsui.chart.AbstractChart chart package}.
 *
 ...
 *                 width: 100                    
 *             };
 *	           me.sparkLine = Gsui.create(config);
 *             me.add(me.sparkLine);
 *
 *             // Put under fields
 ...
 *
 */
Gsui.define('Gsui.sparkline.Base', {
    extend: 'Gsui.Widget',
    requires: [
        'Gsui.XTemplate',
        'Gsui.sparkline.CanvasCanvas',
        'Gsui.sparkline.VmlCanvas'
    ],

    cachedConfig: {
        baseCls: Gsui.baseCSSPrefix + 'sparkline',

        /**
         * @cfg {String} [lineColor=#157fcc] The hex value for line colors in graphs which display lines ({@link Gsui.sparkline.Box Box}, {@link Gsui.sparkline.Discrete Discrete and {@link Gsui.sparkline.Line Line}).
         */
        lineColor: '#157fcc',

        /**
         * @cfg {String} [fillColor=#def] The hex value for fill color in graphs which fill areas ({@link Gsui.sparkline.Line Line}).
         */
        fillColor: '#def',
        ...
        /**
         * @cfg {String/Gsui.XTemplate} [tipTpl] An XTemplate used to display the value or values in a tooltip when hovering over a Sparkline.
         *
    ...
    inheritableStatics: {
        sparkLineTipClass: Gsui.baseCSSPrefix + 'sparkline-tip-target',
        ...
        onClassCreated: function(cls) {
            ...
            for (config in configs) {
                ...
                if (config !== 'tipTpl') {
                    proto[Gsui.Config.get(config).names.apply] = proto.applyConfigChange;
                }
            }    
        }
    },
    ...
    constructor: function(config) {
        ...
        me.canvas = Gsui.supports.Canvas ? new Gsui.sparkline.CanvasCanvas(me)
                                        : new Gsui.sparkline.VmlCanvas(me);
        if (!me.getDisableTooltips()) {
            me.element.cls = Gsui.sparkline.Base.sparkLineTipClass;
        }
    ...
    applyConfigChange: function(newValue) {
        ...
        if (!me.redrawTimer) {
            Gsui.sparkline.Base.prototype.redrawTimer =
                    Gsui.Function.requestAnimationFrame(me.processRedrawQueue);
        }
        return newValue;
    },
    ...
    applyTipTpl: function(tipTpl) {
        if (!tipTpl.isTemplate) {
            tipTpl = new Gsui.XTemplate(tipTpl);
        }
        return tipTpl;
    },
    ...
    getRegionTooltip: function(region) {
        ...
        if (showFields && showFieldsKey) {
            ...
            for (i = fields.length; i--;) {
                ...
                if ((j = Gsui.Array.indexOf(fv, showFields)) != -1) {
    ...
    calcHighlightColor: function(color) {
        ...
        if (lighten) {
            ...
            if (parse) {
                ...
                for (i = 0; i < 3; i++) {
                    rgbnew[i] = Gsui.Number.constrain(Math.round(parseInt(parse[i + 1], 16) * mult * lighten), 0, 255);
                }
    ...
    Gsui.onReady(function() {
        proto.tooltip = new Gsui.tip.ToolTip({
