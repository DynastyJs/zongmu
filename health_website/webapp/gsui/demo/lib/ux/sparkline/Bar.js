/**
 * @class Gsui.sparkline.Bar
 *
 * Plots a bar chart of the values in the passed {@link #values} array.
 *
 * See {@link Gsui.sparkline.Base the base class} for a simple example.
 */
Gsui.define('Gsui.sparkline.Bar', {
    extend: 'Gsui.sparkline.BarBase',
    requires: [
        'Gsui.sparkline.RangeMap'
    ],
    ...
    config: {
        ...
        tipTpl: new Gsui.XTemplate('&#9679; {prefix}{value}{suffix}')
    },
    ...
    applyColorMap: function(colorMap) {
        ...
        if (Gsui.isArray(colorMap)) {
            ...
            if (me.colorMapByValue && me.colorMapByValue.get == null) {
                me.colorMapByValue = new Gsui.sparkline.RangeMap(colorMap);
            }
        }
        return colorMap;
    },
    ...
    onUpdate: function () {
        ...
        for (i = 0, vlen = values.length; i < vlen; i++) {
            ...
            if (isStackString || Gsui.isArray(val)) {
                stacked = true;
                if (isStackString) {
        ...
        for (i = 0, vlen = values.length; i < vlen; i++) {
            if (stacked) {
                ...
                for (j = 0, slen = vlist.length; j < slen; j++) {
                    val = svals[j] = chartRangeClip ? Gsui.Number.constrain(vlist[j], clipMin, clipMax) : vlist[j];
                    if (val !== null) {
                        if (val > 0) {
                            stackTotals[i] += val;
                        }
                ...
                val = chartRangeClip ? Gsui.Number.constrain(values[i], clipMin, clipMax) : values[i];
    ...
    getRegionFields: function (region) {
        var values = Gsui.Array.from(this.values[region]),
            result = [],
            value, i;
    ...
    calcColor: function (stacknum, value, valuenum) {
        ...
        return Gsui.isArray(color) ? color[stacknum % color.length] : color;
    },
    ...
    renderRegion: function(valuenum, highlight) {
        ...
        vals = Gsui.isArray(vals) ? vals : [vals];
