 ...
 */
Gsui.define('Gsui.sparkline.Line', {
    ...
    config: {
        ...
        tipTpl: new Gsui.XTemplate('&#9679; {prefix}{y}{suffix}'),
    ...
    applyValueSpots: function(valueSpots) {
        if (valueSpots && !valueSpots.get) {
            valueSpots = new Gsui.sparkline.RangeMap(valueSpots);
        }
        return valueSpots;
    },
    ...
    renderGraph: function () {
        ...
        if (me.maxy !== me.minyorg) {
            if (spotRadius && minSpotColor) {
                x = xValues[Gsui.Array.indexOf(yValues, me.minyorg)];
            ...
            if (spotRadius && maxSpotColor) {
                x = xValues[Gsui.Array.indexOf(yValues, me.maxyorg)];
                canvas.drawCircle(canvasLeft + Math.round((x - me.minx) * (canvasWidth / rangeX)),
