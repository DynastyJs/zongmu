 ...
 * the chart.
 */
Gsui.define('Demo.view.charts.other.Animated', {
    extend: 'Gsui.Panel',
    ...
    initComponent: function() {
        ...
        var generateData = (function() {
            ...
            return function() {
                data = data.slice(-6);
                data.push({
                    date:  Gsui.Date.add(date, Gsui.Date.DAY, i++),
        ...
        var store = Gsui.create('Gsui.data.JsonStore', {
        ...
        this.callParent();
        chart = Gsui.getCmp('myChartId'); // this.down('cartesian');
        ...
        var addNewData = function() {
            ...
            if (+toDate < +lastDate) {
                ...
                timeAxis.fromDate = Gsui.Date.add(Gsui.Date.clone(timeAxis.fromDate), Gsui.Date.DAY, 1);
                chart.markerIndex = markerIndex;
            }
            store.loadData(gs);
        };

        var task = Gsui.TaskManager.start({
                run: addNewData,
                interval: 1000,
                repeat: 45
        });
