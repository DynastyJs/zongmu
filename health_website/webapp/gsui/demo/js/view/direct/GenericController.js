Gsui.define('Demo.view.direct.GenericController', {
    extend: 'Demo.view.direct.DirectVC',
    alias: 'controller.directgeneric',
    
    requires: [
        'Gsui.direct.PollingProvider'
    ],
    ...
    setInterval: function(field) {
        ...
        if (Gsui.isNumeric(interval) && provider) {
    ...
    onEcho: function(result, event, success, options) {
        ...
        content = Gsui.String.format(
            '<b>Successful call to {0}.{1} with response:</b> <pre>{2}</pre>',
            transaction.action, transaction.method, Gsui.encode(result)
        );
