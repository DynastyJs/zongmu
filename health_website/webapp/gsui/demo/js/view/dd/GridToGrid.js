        ...
        this.items = [{
            ...
            multiSelect: true,
                ...
                listeners: {
                    drop: function(node, data, dropRec, dropPosition) {
                        ...
                        Gsui.example.msg('Drag from right to left', 'Dropped ' + data.records[0].get('name') + dropOn);
                    }
                }
            },
            store: new Gsui.data.Store({
                model: Demo.model.dd.Simple,
                data: this.myData
            }),
            columns: columns,
            title: 'First Grid',
            ...
            viewConfig: {
                ...
                listeners: {
                    drop: function(node, data, dropRec, dropPosition) {
                        var dropOn = dropRec ? ' ' + dropPosition + ' ' + dropRec.get('name') : ' on empty view';
                        Gsui.example.msg('Drag from left to right', 'Dropped ' + data.records[0].get('name') + dropOn);
                    }
                }
            },
            store: new Gsui.data.Store({
