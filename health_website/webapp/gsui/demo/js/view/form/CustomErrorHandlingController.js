Gsui.define('Demo.view.form.CustomErrorHandlingController', {
    ...
    submitRegistration: function() {
        ...
        */
        ...
        if (form.isValid()) {
            var out = [];
            Gsui.Object.each(form.getValues(), function(key, value){
                out.push(key + '=' + value);
            });
            Gsui.Msg.alert('Submitted Values', out.join('<br />'));
        }
    },
    ...
    updateErrorState: function(cmp, state) {
        ...
        if (state === false || (typeof state === 'string' && state.length)) {
            ...
            fields.each(function(field) {
                Gsui.Array.forEach(field.getErrors(), function(error) {
