/**
 * This is a more advanced example that shows how you can combine Gsui.Template and a
 ...
 * forum thread.
 */
Gsui.define('Demo.view.form.ForumSearch', {
    extend: 'Gsui.panel.Panel',
    xtype: 'form-forumsearch',
    requires : ['Demo.store.form.ForumPosts'],
    //<example>
        ...
        listConfig: {
            ...
            itemTpl: [
                '<a class="search-item" href="http://www.sencha.com/forum/showthread.php?t={topicId}&p={id}">',
                    '<h3><span>{[Gsui.Date.format(values.lastPost, "M j, Y")]}<br />by {author}</span>{title}</h3>',
                    '{excerpt}',
                '</a>'
            ]
        }
