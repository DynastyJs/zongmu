Gsui.define('Demo.controller.Global', {
    extend: 'Gsui.app.Controller',
    requires: [
        'Demo.view.*','Demo.DummyText',
        'Gsui.window.*'
    ],
    ...
    stores: [
        ...
        'StockPrice',
        'LinearGeoData',
        'EconomySectors','Browsers','Climate','TwoYearSales','Earnings','Plot','GDP','DeviceMarketShare','Gauges','Unemployment'//charts
    ],
    ...
    beforeHandleRoute: function(id, action) {
        var me = this,
            node = Gsui.StoreMgr.get('navigation').getNodeById(id);
    ...
    handleRoute: function(id) {
        ...
        if (node.isLeaf()) {
            ...
            contentPanel.body.addCls('Demo-example');
			Gsui.syncRequire('Demo.view.'+node.get('classPath'));
            className = Gsui.ClassManager.getNameByAlias('widget.' + id);
           /**if(Gsui.isEmpty(className)){
            	var arrClassName = id.split("-");
            	className = 'Demo';
            	if(arrClassName && arrClassName.length > 0){
            		for(var i = 0;i < arrClassName.length;i++){
            			if(i == arrClassName.length-1){
            				...
            				className += '.'+Gsui.String.capitalize(arrClassName[i]);
            		}
            	}
            	className = className.substring(className[length-2]);
            }**/
            ViewClass = Gsui.ClassManager.get(className);
            ...
            if (clsProto.themes) {
                ...
                if (themeName === 'gray') {
                    clsProto.themeInfo = Gsui.applyIf(clsProto.themeInfo || {}, clsProto.themes.classic);
                } else if (themeName !== 'neptune' && themeName !== 'classic') {
                    if (themeName === 'crisp-touch') {
                        clsProto.themeInfo = Gsui.applyIf(clsProto.themeInfo || {}, clsProto.themes['neptune-touch']);
                    }
                    clsProto.themeInfo = Gsui.applyIf(clsProto.themeInfo || {}, clsProto.themes.neptune);
                }
                // <debug warn>
                // Sometimes we forget to include allowances for other themes, so issue a warning as a reminder.
                if (!clsProto.themeInfo) {
                    Gsui.log.warn ( 'Example \'' + className + '\' lacks a theme specification for the selected theme: \'' +
            ...
            contentPanel.body.removeCls('Demo-example');
    ...
    setupPreview: function(clsProto) {
        ...
        if (!codePreviewProcessed) {
            resources.push({
                type: 'View',
                path: clsProto.$className.replace(/\./g, '/').replace('Demo', 'js') + '.js'
            });
            ...
            codePreviewProcessed = clsProto.codePreviewProcessed = [];
            Gsui.each(resources, function(resource) {
                ...
                resource.loader = {
                    ...
                    failure : function(loader,response,req){
                    	if(response && response.responseText){
                    		loader.getRenderer(loader.renderer).call(loader.rendererScope, loader, response);
                    	}
                    }
                };
            });
    ...
    processText: function (text, themeInfo) {
        var lines = text.split('\n'),
            ...
            encodeTheme = function (text, match) {
                return Gsui.encode(themeInfo[match]);
            },
            i, line, code;
        ...
        code = Gsui.htmlEncode(keepLines.join('\n'));
