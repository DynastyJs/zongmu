 ...
 */
Gsui.define('Gsui.ux.IFrame', {
    extend: 'Gsui.Component',
    ...
    initRenderData: function() {
        return Gsui.apply(this.callParent(), {
            src: this.src,
    ...
    cleanupListeners: function(destroying){
        ...
        if (this.rendered) {
            try {
                doc = this.getDoc();
                if (doc) {
                    Gsui.get(doc).un(this._docListeners);
                    if (destroying) {
                        for (prop in doc) {
    ...
    onLoad: function() {
        ...
        if (doc) {
            try {
               ...
               this.frameEl.on('click',function(){alert('click')});
            } catch(e) {
                // cannot do this xss
            }

            // We need to be sure we remove all our events from the iframe on unload or we're going to LEAK!
            Gsui.get(this.getWin()).on('beforeunload', me.cleanupListeners, me);
 ...
 * TODO items:
 *
 * Iframe should clean up any Gsui.dom.Element wrappers around its window, document
 * documentElement and body when it is destroyed.  This helps prevent "Permission Denied"
 * errors in IE when Gsui.dom.GarbageCollector tries to access those objects on an orphaned
