...
var serverTable = {
	...
	setTable : function() {
		...
		var settings = {
			...
			queryParams : function(params) {
				...
				if($("#equType").val()!=-1){
					params['search_LIKE_equipmentTypeName'] = $("#equType").val() || "";
				}
	...
	refreshTb : function() {
		$("#bootstrapTable").bootstrapTable('refresh', {
			url : Constants.CONTEXT_PATH + '/ring/getPageList.do'
		});
	},
	/**
	 * 查询数据
...
var uiAlarmTable = {
	...
	setTable : function() {
		...
		var settings = {
			...
			queryParams : function(params) {
				...
				params.pageSize=params.limit;
				params.pageNumber=pageNumber;
				params['search_IN_propertyName'] =  "市电,UPS,温度,湿度";
				params['search_ORGLIKE_path'] = treeSelectOrgId;
				return params;
			}
		}
		C.createTable("#bootstrapTable_uialarm", Constants.CONTEXT_PATH
						+ '/uialarm/getPageList.do', settings);
	},
	/**
	 * 更新表格
	 */
	refreshTb : function() {
		$("#bootstrapTable_uialarm").bootstrapTable('refresh', {
			url : Constants.CONTEXT_PATH + '/uialarm/getPageList.do'
		});
	},
	/**
	 * 查询数据
...
window.operateEvents = {
	'click .dealBtn' : function(e, value, row, index) {
		popUpWindow.handleEvent(row.fsuId,row.propertyName);
	},
...
var popUpWindow = {
	 ...
	 * 处理
	 */
	handleEvent : function(fsuid,propertyName) {
		if (typeof(fsuid) == 'undefined' || fsuid == '') {
			layer.alert('请选择需要处理的记录!', {icon : 0});
		} else if (isNaN(fsuid)) {
			...
			$.ajax({
						async : false,
						url : Constants.CONTEXT_PATH + '/ring/findByFsuIdAndProperty.do',
						data : {
							fsuid : fsuid,
			...
			var d = dialog({
						title : '处理',
						content : document.getElementById('ringwin'),
