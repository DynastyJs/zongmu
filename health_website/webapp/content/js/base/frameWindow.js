...
$.fWin = function(options){
   ...
   function newWin(){
   		...
   		if(settings.maskLayer) {
   			if(settings.modal){
   				var iframeHtml = "<div src='about:blank' style=\"border:0;position:absolute; visibility:inherit; top:0px; left:0px;  height:100%;width:100%; z-index:1000; background-color:#ccc;opacity:0.5;filter='progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0.5)'\"/>";
				win.modalMask = $(iframeHtml).appendTo($('body'));
   			}
			var iframeHtml = "<iframe src='about:blank' style=\"border:0;position:absolute; visibility:inherit; top:0px; left:0px;  height:100%;width:100%; z-index:-1; filter='progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)'\"/>";
   ...
   function setTitle(win){
   	if(settings.showTitle){
   		var str = '<div class="fwn-title" style="height:'+settings.titleHeight+'px;line-height:'+settings.titleHeight+'px;"><span style="float:left;margin-left:10px;">'+settings.title+'</span></div>';
   		...
   		if(settings.needFooter){
	   		...
	   		if(settings.buttons){
	   			for(var i=0;i<settings.buttons.length;i++){
					  var b = '<button class="btn btn-primary fw-btn" >'+settings.buttons[i].text+'</button>';
					  b = $(b).appendTo(footer);
					  if(settings.buttons[i].callback){
					  	var callback = settings.buttons[i].callback;
						 b.click(function(){
