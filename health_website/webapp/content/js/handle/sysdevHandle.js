...
function initData(){
	var netStatus ='',cpu = '',memory = '',memory_size='',diskspace='',equipmentName='',netAddress='',producer='';
	$.ajax({
		async : false,
				...
				success: function (data) {
					...
					for (var i = 0; i < data.data.length; i++) {
						...
						if(data.data[i].alarmEventViewID == ""){
							ret =  '<span style="width:130px;text-align:left;display:inline-block;color:#666;">'+name+ ':</span><span style="width:100px;display:inline-block;text-align:left;color:#333;" title="'+data.data[i].propertyValue+'">' + value +'</span>';
						}else{ //这种状态有告警
							ret = "<span style='width:130px;text-align:left;display:inline-block;color:red;'>"+name + ':</span><span style="width:100px;display:inline-block;text-align:left;" title="'+data.data[i].propertyValue+'"> ' + value  +"</span>";
						}
						morehtml+=ret;
						if(i%2==0){
							morehtml+="</br>";
