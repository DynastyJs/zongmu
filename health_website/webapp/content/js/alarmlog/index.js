...
function autoRecovFormaterTime(value,row,index){
	if(row.processTimeStr==""){
		return row.endTimeStr;
	}
	return row.processTimeStr;
}

function autoRecovFormater(value,row,index){
	if(row.processTimeStr==""){
		return "自动恢复";
	}else{
		return value;
...
function initEvent() {
	$('#processResultSearch').append(createSearchSelectOptions('DEAL_RESULT'));
