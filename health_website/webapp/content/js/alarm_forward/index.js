...
function initLeftData(){
	...
	$.ajax({
		...
		data:{
			pageSize:10000,
			search_EQ_isAlarmRule:1
			},
		dataType:'json',
		success:function(res){
			if(res){
				var data = res.content;
				$('.timeTemplate').html('');//清空下拉框选项
				for(var i=0;i<data.length;i++){
					$('.list-group').append(createHtml(data[i].name,i+1,data[i].id));
					
					$('.timeTemplate').append('<option value="'+data[i].id+'">'+data[i].name+'</option>');
				}
				if(data.length>0){
				    //切换规则
				    $(".rule").click(function () {
...
function initRule(){
	...
	$.ajax({
		async:true,
		url:Constants.CONTEXT_PATH+'/alarmForward/getAllRule?fresh=' + Math.random(),
		dataType:'json',
		success:function(rules){
			...
			for(var i in rules){
				var rule = rules[i];
				$('[data-alarm-level="'+rule.alarmLevel+'"]').each(function(){
					$(this).data('rule-id',rule.id);
					$(this).find('.timeTemplate').val(rule.forwardTimeTemplate);
					$(this).find('.readyTime').val(rule.delayTime);
					$(this).find('.switch').bootstrapSwitch('destroy').bootstrapSwitch({
						onText : "启动",
						offText : "关闭",
						onColor : "success",
						offColor : "info",
						size : "small",
						state:rule.isEnable==1,
						onInit: function(){
							$(this).val(rule.isEnable);
						},
						onSwitchChange : function(event, state) {
							if (state == true) {
								$(this).val(1);
...
function initEvent(){
    ...
    $('#saveBtn').click(function(){
    	if(validator.isRulePass()){
    		var arr = [];
    		var levels = ['紧急','重要','普通'];
    		for(var i in levels){
    			var $ele = $('[data-alarm-level="'+levels[i]+'"]');
    			arr.push({
    				id:$ele.data('rule-id')||(i*1+1),
    				alarmLevel:levels[i],
    				forwardTimeTemplate:$ele.find('.timeTemplate').val(),
    				delayTime:$ele.find('.readyTime').val(),
    				isEnable:$ele.find('.switch').val()
    			});
    		}
    		$.ajax({
    			url:Constants.CONTEXT_PATH+'/alarmForward/update?fresh=' + Math.random(),
    			async:false,
    			type:'POST',
    			dataType:'json',
    			data:{rulesStr:JSON.stringify(arr)},
    		...
    		layer.alert('延迟时间为0-3600的整数',{icon: 0});
    	}
    });
}
				...
				success:function(data){
					...
					for(var i=0;i<param.length;i++){
						...
						for (var j = 0; j < param[i].listTimeTemplateParamSpan.length; j++) {
							...
							var startTime = (startDate.hours<10?('0'+startDate.hours):startDate.hours)+":"+(startDate.minutes<10?('0'+startDate.minutes):startDate.minutes);
...
//		});
	...
	isRulePass:function(){
		...
		$('.readyTime').each(function(){
			var time = $(this).val();
			var reg=/^\+?[1-9][0-9]*$/;
			if(!(reg.test(time)&&time>=0&&time<=3600)){
				flag = false;
				return false;
			}
		});
...
//    }
    ...
    var data = { "id":$("input[name='ruleCode']").val()||-1,"name": ruleName,"saveDays":$("input[name='saveDays']").val()||"", "isPublic": $("input[name='Enable']").is(":checked")?1:0, "plans": JSON.stringify(times),"isAlarmRule":1 };
	$.ajax({
...
//        var ruleType = $("select[name='SelType" + weekValue + "_" + j + "']").val();
        if (startTime && endTime && ((startTime != defaultTime || endTime != defaultTime)))//开始时间和结束时间不等于默认时间,切类型存在
        {
            ...
            if (parseInt(eval(startTimes[0].toString())) == 0) {
                start = parseInt(eval(startTimes[1].toString()));
            }
            else {
                start = parseInt(eval(startTimes[0].toString())) * 60 + parseInt(eval(startTimes[1].toString()));
            }

            if (parseInt(eval(endTimes[0].toString())) == 0) {
                end = parseInt(eval(endTimes[1].toString()));
            }
            else {
                end = parseInt(eval(endTimes[0].toString())) * 60 + parseInt(eval(endTimes[1].toString()));
            }
